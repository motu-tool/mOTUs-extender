...
def predictGenes(contigsFasta, metagenomicMode, transTableNumber, protOut, nuclOut, gffOut):
    ...
    cmd = "prodigal " + "-a " + str(protOut) + " -d " + str(nuclOut) + " -f gff -o " + str(gffOut) + " -c" + str(transTableParameter) + " -q -m -p " + metagenomicMode + " -i " + contigsFasta    
    print(cmd)
...
def runFetchMGs(protFasta, numThreads, outMGfolder, fetchMGfolder):
    ...
    #os.system(cmd) 
    #cmd = "fetchMGs.pl -p -m extraction " + "-t " + str(numThreads) + " -o " + str(outMGfolder) + " " + str(protFasta)
    cmd = "fetchMGs.pl -i -v -p -m extraction " + "-t " + str(numThreads) + " -o " + str(outMGfolder) + " " + str(protFasta)
    if (fetchMGfolder != ""):
        cmd = os.path.sep.join([fetchMGfolder,cmd])
...
def parseFetchMGs(outMGfolder, OGType, runPrefix):
    setMGs = set()
    mgID_genome_mgs = defaultdict(lambda : "COG0000")
    
    if (OGType == "mOTU"):
        OGs2use = ["COG0012", "COG0016", "COG0018", "COG0172", "COG0215", "COG0495", "COG0525", "COG0533", "COG0541", "COG0552"]
    fetchMG_table = os.path.sep.join([outMGfolder,runPrefix+"*.table"])
    fetchMG_table_path = glob.glob(fetchMG_table)
    with open(fetchMG_table_path[0],'r') as currentFile:
        for strLine in currentFile:
            ...
            if ( currOG in OGs2use or "all" in OGs2use):
                setMGs.add(mgID)
                mgID_genome_mgs[mgID] = currOG        
                
    return(setMGs, mgID_genome_mgs)
...
def parseGeneLocations_gff(setMGs, gffFileName, padLength, paddedLocationsFileName, extractLocationsFileName, geneLengthFileName, paddedLengthFileName, mgID_genome_mgs, runPrefix):
    
    gffFile = open(gffFileName, "r")
    print("Parsing GFF file.")
    paddedLocationsFile = open(paddedLocationsFileName, "w")
    print_path_file("Padded Location", paddedLocationsFileName)

    extractLocationsFile = open(extractLocationsFileName, "w")
    print_path_file("extract Location", extractLocationsFileName)
    geneLengthFile = open(geneLengthFileName, "w")
    print_path_file("Genes length", geneLengthFileName)
    paddedLengthFile = open(paddedLengthFileName, "w")    
    print_path_file("Padded length", paddedLengthFileName)
    paddedLocationsFileSpace = open(paddedLocationsFileName + ".ssv" , "w")
    dictExtractName2writeName = defaultdict(list)
    padLength = int(padLength)
    ...
    for strLine in gffFile: 
        ...
        if(strLine.startswith("# Sequence Data:")):
            ...
            for infoPart in [infoPart for infoPart in infoList if '=' in infoPart]:
            ...
            if (geneName in setMGs or "all" in setMGs):    
                contig_gene_mg = geneName + '.' + mgID_genome_mgs[geneName]
                genome_mg = runPrefix + '.' +  mgID_genome_mgs[geneName]
                #geneName contigID ID in padded file start of gene on padded seq 
                writeline = genome_mg + "\t" + contigID + "\t" + str(newGeneStart) + "\t" + str(newGeneEnd) + "\n"
                paddedLocationsFile.write(writeline)    
                writeline = genome_mg + " " + contigID + " " + str(newGeneStart) + " " + str(newGeneEnd) + "\n"
                paddedLocationsFileSpace.write(writeline)        
                ...
                lengthLine = genome_mg + "\t" + str(length)
                geneLengthFile.write(lengthLine + "\n")
                
                lengthLine2 = genome_mg + "\t" + str(paddedEnd - paddedStart + 1)
                paddedLengthFile.write(lengthLine2 + "\n")    
                
                paddedGeneName = genome_mg + " " + str(newGeneStart) + " " + str(newGeneEnd)
                dictExtractName2writeName[extractLine].append(genome_mg)
...
def getPaddedSequences(extractLocationsFileName, contigsFasta, paddedFastaFileName, dictExtractName2writeName): 
    ...
    print(' '.join(['cat', extractLocationsFileName, '|','xargs', 'samtools', 'faidx', contigsFasta]))
...
def print_path_file(type_file, path):
    to_print = type_file+" path: "+path
    print(to_print)

def runMakePaddedMG_DB(contigsFasta, outfolder, runPrefix, gffFileName, contigCoordsFileName, contigLengthsFileName, padLength, geneNamesFileName, proteinFasta, transTableNumber, metagenomicMode, numThreads, OGType, fetchMGfolder):
    ...
    if (proteinFasta and not geneNamesFileName):
        ...
        runFetchMGs(proteinFasta, numThreads, outMGfolder, fetchMGfolder)
        print("OGType: ", OGType)
        setMGs, mgID_genome_mgs = parseFetchMGs(outMGfolder, OGType, runPrefix)
        print_path_file('FetchMG output', outMGfolder)
        print('setMGs: ', setMGs)
        print("...done")
    ...
    #this part is about getting the locations of the genes from either provided files or from gff files from geneprediction 
    if (gffFileName):
        dictExtractName2writeName = parseGeneLocations_gff(setMGs, gffFileName, padLength, paddedLocationsFileName, extractLocationsFileName, geneLengthFileName, paddedLengthFileName, mgID_genome_mgs, runPrefix)
...
def main(argv=None):
    ...
    parser.add_argument('--metagenomicMode', '-m',action="store", dest='metagenomicMode', default="meta", choices=["meta", "single"], help="Select a procedure for prodigal gene prediction, default is meta; alternative is 'single'.")
    parser.add_argument('--fetchMGfolder', '-f',action="store", dest='fetchMGfolder', type=str, default="", help="Folder that hosts fetchMGs (if not in path).")
