...
def initMatrices(listKeys):
	...
	print(numKeys)
	distanceMatrix = scipy.sparse.dok_matrix((numKeys,numKeys), dtype=numpy.float32)
	distanceMatrixEdges = scipy.sparse.dok_matrix((numKeys,numKeys), dtype=int)
		...
		if (strID_1 in dictKey2Position and strID_2 in dictKey2Position):
			...
			if not (tuplePositions in setMatrixEntries):
				...
				if (type == "ID"):
					total = 100.0
					newtotal = 1.0
					floatDist = (float(total) - floatDist) * (float(newtotal)/float(total))
...
def getConnectedComponents(distanceMatrixEdges, listKeys):
	...
	N_components, component_list = scipy.sparse.csgraph.connected_components(distanceMatrixEdges)
...
#array([-2,  0,  0,  0,  1,  1,  1, -2,  2,  2], dtype=int32)
	...
	print("Preclustering results: Connected Components: " + str(N_components) + "\tSingletons: " + str(len(listSingletons)) )
...
def getSubClustering(listIDs_subcluster, setClusters, distanceMatrix, strLinkMethod, strCutoffMethod, floatCutoff, dictCluster2IDs, dictID2Cluster, listKeys, forceClusterSeparation):
	...
	matrixInternalDist = numpy.ones((numberOfIDs, numberOfIDs),dtype=numpy.float32)
	#print(numberOfIDs)
...
#		internalKeyCount += 1	
	...
	vectInternalDistMatrix = scipy.spatial.distance.squareform(matrixInternalDist)
	linkDistMatrix = module_cluster.linkage(vectInternalDistMatrix, method=strLinkMethod)
	del vectInternalDistMatrix
	vectorClusters = scipy.cluster.hierarchy.fcluster(linkDistMatrix, floatCutoff, strCutoffMethod)
	...
	for cluster in vectorClustersOffset:
		if (cluster in setClusters):
			...
			vectorClustersOffset = [x + maxClusterNumber + 1 for x in vectorClusters]
...
def runClustering(listIDsFileName, distFileName, floatCutoff, type, strLinkMethod, strCutoffMethod, noConnectedComponents, forceClusterSeparation, preClusteringFileName, preClusterMapFile, outfileName):
	...
	if (type == "ID"):
		total = 100.0
		newtotal = 1.0
		floatCutoff = (float(total) - floatCutoff) * (float(newtotal)/float(total))
	...
	if (preClusteringFileName != ""):
		(dictCluster2Members, dictID2Cluster, listClusterNames) = parseClustering(preClusteringFileName, prefix = "")
		hasPreClustering = True
		if (preClusterMapFile != ""):
			(dictOldIDs2NewIDs, dictNewIDs2OldIDs) = parse2columnFileDictLists(preClusterMapFile)
			dictID2Cluster = reformatDict(dictID2Cluster, dictOldIDs2NewIDs, listIDs)
			dictCluster2Members = reformatDict2(dictCluster2Members, dictOldIDs2NewIDs, listIDs)
	else:
		dictID2Cluster = {}
		dictCluster2Members = {}
	...
	(distanceMatrixEdges, distanceMatrix) = fillDistMatrix(distFileName, distanceMatrixEdges, distanceMatrix, dictKey2Position, dictID2Cluster, dictCluster2Members, type, listIDs, floatCutoff, forceClusterSeparation, hasPreClustering)
	if (not noConnectedComponents):
		(dictComponent2IDsNumber, listSingletons) = getConnectedComponents(distanceMatrixEdges, dictKey2Position)
	else:
		listSingletons = []
		dictComponent2IDsNumber = {}
		dictComponent2IDsNumber["All"] = [index for index, item in enumerate(listIDs)]
	...
	for component in dictComponent2IDsNumber:
		...
		if (len(listIDsInComponent) > 1 ):
			...
			dictCluster2IDs, setClusters = getSubClustering(listIDsInComponent, setClusters, distanceMatrix, strLinkMethod, strCutoffMethod, floatCutoff, dictCluster2IDs, dictID2Cluster, listIDs, forceClusterSeparation)
	...
	for cluster in dictCluster2IDs:
		...
		strWriteLine = str(cluster) + "\t" + ";".join(listClusterIDsOriginalIDs) + "\n"
...
def main(argv=None):
	...
	parser = argparse.ArgumentParser(description='This program is used to cluster sequences into clusters based on sequence identity values', add_help = True)
	parser.add_argument('distancefile', action="store", help='file with distances or similarities in format ID1\tID2\tdist/sim')
	parser.add_argument('listIDsFileName', action="store", help='file listing all IDs to be used for the clustering')
	parser.add_argument('cutoff', action="store", type=float, help='cutoff for the clustering')
	parser.add_argument('outfile', action="store", help='prefix for outfile(s)')
	parser.add_argument('--distOrID', '-d', action='store', dest='type', default="ID", help="'dist' or 'ID'; if 'ID' then its transformed into a distance inside, defaults to 'ID' [ID_1]")
	parser.add_argument('--linkMethod', '-l', action='store', dest='linkMethod', default='average', help="linkMethod: 'single','complete','average','weighted', defaults to average")
	parser.add_argument('--noConnectedComponents', '-n', action='store_true', default=False, dest='noConnectedComponents', help='Set to not use the connected component preclustering')
	parser.add_argument('--forceClusterSeparation', '-s', action='store_true', default=False, dest='forceClusterSeparation', help='Force preclusters to stay separate from each other')
	parser.add_argument('--preClusteringFile', '-p', action='store', dest='preClusteringFile', default="", help='File with a preclustering e.g. specI clusters. The preclusters will be kept.')
	parser.add_argument('--preClusterMap', '-m', action='store', dest='preClusterMapFile', default="", help='Tab delimited 2 column file mapping IDs in the preclustering to IDs in the distance file. E.g. specI clusters to geneID in the currently clustered OG')
	parser.add_argument('--fastcluster', '-f', action='store_true', dest='fastcluster', default=False, help='Set to use fastcluster module for clustering')
	...
	runClustering(args.listIDsFileName, args.distancefile, args.cutoff, args.type, args.linkMethod, strCutoffMethod, args.noConnectedComponents, args.forceClusterSeparation, args.preClusteringFile, args.preClusterMapFile, args.outfile)	
	return 0		# success
